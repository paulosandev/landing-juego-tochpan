...
export type AnimationStateName = 'idle' | 'running' | 'jumping' | 'landing';
...
export interface AnimationState {
  ...
  frameRate: number;
  loop: boolean;
  duration?: number; // Optional duration in ms for non-looping animations
}

// Animation configuration as specified in design document
export const ANIMATION_CONFIGS: Record<AnimationStateName, AnimationState> = {
  idle: {
    name: 'idle',
    frames: [0],
    frameRate: 1,
...
export class AnimationController {
   ...
   */
  setState(stateName: AnimationStateName): void {
   ...
   */
  update(deltaTime: number, gameSpeed: number = 1.0): void {
    ...
    if (this.frameAccumulator >= frameDuration) {
      ...
      if (this.currentState.loop) {
        ...
        this.currentFrame = Math.min(
          ...
          this.currentState.frames.length - 1
        );
      }
    }
   ...
   */
  getStateName(): AnimationStateName {
   ...
   */
  private calculateEffectiveFrameRate(gameSpeed: number): number {
